...
Thanks for using `eia-energy-mcp-server`. Bugs, feature requests, and documentation gaps all belong in an issue — that's where they get read and picked up.

- [Report a bug](https://github.com/cyanheads/eia-energy-mcp-server/issues/new?template=bug_report.yml)
- [Request a feature](https://github.com/cyanheads/eia-energy-mcp-server/issues/new?template=feature_request.yml)
...
`eia-energy-mcp-server` is built on [@cyanheads/mcp-ts-core](https://github.com/cyanheads/mcp-ts-core), which handles transports, auth, config, logging, and telemetry. Sorting out which layer broke saves everyone a round-trip:

- **This repo** — a tool returns wrong data, a call to the EIA API v2 fails, a schema doesn't match reality, a description misleads the model.
...
3. **Redact anything sensitive.** Issues are public and permanent — no API keys, tokens, auth headers, or internal URLs in code, logs, or stack traces. `EIA_API_KEY` rides in request URLs as an `api_key` query parameter, so scrub those before pasting.

## What makes an issue actionable

- Server version, `mcp-ts-core` version, runtime (Bun / Node / Workers), and transport (stdio / HTTP).
- The route path involved (e.g. `electricity/retail-sales`) and the facet filters, frequency, and date range you passed.
