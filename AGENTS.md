...
const ServerConfigSchema = z.object({
  ...
  dataframeDropEnabled: z.stringbool()
...
Mark service-produced reasons with `thrownBy: 'service'`; forward every handler-local failure hint with `ctx.recoveryFor(reason)`. Both are lint-checked.
...
**HTTP sessions:** `createApp({ sessionMode: 'stateless' })` is the durable default; `.env.example` and Docker agree. No tool uses `ctx.requestInput`. An explicit `MCP_SESSION_MODE` overrides the default; `auto` is the framework schema default and resolves to `stateful`. Tenant storage and DataCanvas are independent of HTTP sessions.

**Route argument aliases:** browse retains its advertised `path` key and accepts `route`; describe and query retain `route` and accept `path`. `inputAliases` handles this before validation without changing advertised schemas. Calls supplying both spellings are rejected.
...
| `git-wrapup` | Land the commit stack and open the release PR; no tag or push to main. |
| `release-pr-review` | Review a gated release PR and land corrections as ordinary commits. |
...
| `bun run audit:refresh` | Last resort after `audit:fix`, a targeted update, and dedupe: re-resolves every ranged dependency, including the framework pin. |
...
`bun run bundle` produces a `.mcpb` extension bundle for one-click install in Claude Desktop. The pack step is followed by `scripts/clean-mcpb.ts`, which prunes dev dependencies (`mcpb clean`) and strips two classes of `node_modules/**` content that root-anchored `.mcpbignore` patterns cannot reach: dependency-shipped agent docs (`skills/`, `.claude/`, `.agents/`, `SKILL.md`) and platform-specific native bindings, which would otherwise lock the bundle to the platform it was packed on. This server uses DataCanvas, so that second class matters — `@duckdb/node-api` is an optional peer loaded lazily, and the bundle ships portable without the DuckDB native: canvas tools report an actionable install hint and every other tool works normally. MCPB is stdio-only — HTTP deployments are unaffected. Delete `manifest.json` and `.mcpbignore` if not needed; `lint:packaging` skips cleanly.
...
- [ ] `.codex-plugin/mcp.json` updated — server name key is the unscoped repo name; user-supplied variables are forwarded with `env_vars`, never empty `env` entries
- [ ] `.claude-plugin/plugin.json` populated — `name`, `version`, `description`, `repository`, `license` from `package.json`; inline `mcpServers` entry keyed by the unscoped repo name; required API keys declared in `userConfig` and referenced as `${user_config.<option>}`
