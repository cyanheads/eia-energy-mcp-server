...
4 route tools, always registered, plus 3 dataframe tools registered only when a canvas is configured (`CANVAS_PROVIDER_TYPE=duckdb`) — one of which, `eia_dataframe_drop`, needs its own opt-in on top. The default deployment surface is the 4 route tools.
...
| `eia_query_route` | Fetches data from a leaf route with optional facet filters, date range, frequency, and column selection. Returns a preview inline and stages nothing by default; with `stage: true` it also pages past the preview up to `EIA_CANVAS_MAX_ROWS` and returns `dataset` (`df_<id>`) for SQL analysis. | `route`, `filters?` (facet key-value pairs), `start?`, `end?`, `frequency?`, `columns?`, `sort?`, `offset?`, `length?`, `stage?` | `readOnlyHint`, `openWorldHint: false` |
| `eia_dataframe_describe` | List canvas dataframes materialized by `eia_query_route` calls that passed `stage: true`, with provenance, expiry, row count, and column schema. A `name` that is not staged returns `found: false` alongside the handles that are. Listing does not extend a dataframe's expiry. | `name?` (single `df_<id>` or omit for all) | `readOnlyHint`, `idempotentHint`, `openWorldHint: false` |
...
| `eia_dataframe_drop` | Drop a canvas dataframe by name. **Opt-in** via `EIA_DATAFRAME_DROP_ENABLED=true`, on top of the canvas gate every dataframe tool carries — off by default since the sliding expiry handles cleanup. Idempotent: returns `dropped=false` when nothing matched. | `name` | `readOnlyHint: false`, `idempotentHint`, `destructiveHint: true` |
...
- DataCanvas (DuckDB) for tabular staging — opt-in, Node only; `eia_query_route` degrades to preview-only when unavailable (`ctx.core.canvas` is undefined in Workers) and the dataframe tools are gated out of the surface at registration
...
**Route path normalization:** `browse()`, `describe()`, and `query()` each normalize the path before resolving it — leading and trailing slashes stripped, internal runs collapsed. EIA's own docs and data browser write routes as `/v2/electricity/retail-sales/`, so an agent working from upstream documentation rather than from `eia_browse_routes` output writes the slashes too. Without normalization the three tools disagreed: `browse` resolves only against the node map, whose keys are bare, so every variant missed; `describe` and `query` missed the map as well but fell through to a live EIA fetch, which EIA normalizes server-side, so they succeeded. Normalizing once at the service boundary also collapses what were three metadata-cache entries and three facet fan-outs per route into one, and restores the cached leaf/category pre-flight for every spelling — a category variant now fails fast as `route_not_queryable` instead of spending an upstream round trip to learn the same thing. The canonical value travels back on `RouteMetadata.route` and `DataResponse.route`, so the tools echo the spelling a caller can reuse rather than the one they typed.
...
| `CANVAS_PROVIDER_TYPE` | No | Set to `duckdb` to enable DataCanvas (Node only; Workers fail closed). The three dataframe tools are registered only when it is set, and `eia_query_route` can stage only when it is |
...
| `EIA_DATAFRAME_DROP_ENABLED` | No | Set to `true` to expose `eia_dataframe_drop`, which also requires a canvas. Default `false`; TTL handles cleanup in normal operation. |
| `EIA_CANVAS_MAX_ROWS` | No | Cumulative row ceiling for `eia_query_route` canvas accumulation on a `stage: true` call. Default `25000` — five requests at EIA's 5,000-row-per-request ceiling, ~4 MB of upstream JSON and ~8.5 s of tool latency when it binds. Lower it to keep exploratory calls snappy, raise it for wider staged analyses. |
...
6. `eia_query_route` — filters, pagination, opt-in DataCanvas staging (`stage: true` plus `ctx.core.canvas?`); emit `dataset` (`df_<id>`) when rows were staged
...
9. Conditional registration in `createApp()` — all three dataframe tools gated on `CANVAS_PROVIDER_TYPE=duckdb`, `eia_dataframe_drop` additionally on `EIA_DATAFRAME_DROP_ENABLED`. `createApp()` composes its `tools[]` before `setup(core)` runs, so `core.canvas` is not in hand; the gate reads `process.env.CANVAS_PROVIDER_TYPE` directly rather than joining core config to `ServerConfigSchema`
...
**Missing labels upstream:** EIA sends some facet values with a null `id`, others with a null or absent `name`, and the service treats the two differently. The `id` is the filter value, so a value without one is dropped. A value without a `name` keeps its `id` and takes its label from `alias`, falling back to the `id` when there is no alias either. Such a value renders as `id=alias`: the suppression rule above finds the alias restating the name it was just copied into and prints it once.
...
A `{col}-units` companion is not always constant down the column — on routes where the unit follows a facet value, `electricity/electric-power-operational-data` and `electricity/facility-fuel` among them, one row's unit does not speak for the rest. `structuredContent` carries the raw per-row field either way; `format()` decides per companion from the whole preview. Identical on every row, it folds into the header as `col (unit)` and leaves the body one column narrower. Absent from every row, it is dropped — the body column would be blank. Anything else — varying, or null on some rows only — stays as its own body column, so the markdown never asserts a unit the rows contradict.
...
The `incomplete return` entry is the one EIA sends on every page narrower than `total`, and its 5,000-row text is fixed boilerplate rather than a real ceiling report — it fires at 500 rows of 1,830 as readily as at 25,000 of 113,460. The tool drops it, and only it, on a response that already states where the caller stands: a `notice` explaining a row-less page, a staged table running from the caller's `offset` to the last row, or — where nothing was staged, whether because `stage` was not set or because no canvas is configured — a `canvas_preview_note` placing the inline page against `total`. Any other advisory, and this one whenever staging stopped short, still reaches `truncation_warning` — matching is on the `warning` label, so an entry EIA adds later forwards by default.

**Canvas accumulation:** Staging is opt-in per call. With `stage: true`, a canvas configured, and more rows matching than the preview holds, the service walks `offset` pages forward from the preview — each page bounded by EIA's 5,000-row-per-request ceiling — and returns the accumulated set separately from the inline preview. The inline `data` array always stays at the caller's `length`; accumulation only widens what reaches the canvas. `EIA_CANVAS_MAX_ROWS` (default 25,000) bounds the cumulative total; when it binds, `canvas_preview_note` names the real staged row count, the cap, and the offset to resume from. Accumulation is best-effort, matching the canvas bridge it feeds: a follow-up page that fails after retries stops staging and keeps the rows already gathered rather than discarding a preview the caller has in hand, and `canvas_preview_note` reports the shortfall and the resume offset without blaming the cap. A caller-side abort still propagates. Requesting a page size above 5,000 is not an upstream error — EIA returns the first 5,000 rows plus a `parameter out of range: length` warning — so the input schema's `.max(5000)` owns that bound and the service carries no redundant guard.

**Input schema:**
- `route: string` — Leaf route path (e.g. `"electricity/retail-sales"`). Leading, trailing, and doubled slashes are stripped at the service boundary
...
- `length?: number` — Rows in the inline preview (default 100, max 5000). With `stage: true`, staging pages past this on its own.
- `stage?: boolean` — Stage the matching rows as a DataCanvas table for SQL (default `false`). Off, the call makes one upstream request and returns the preview alone.

**Output:**
- `route: string` — the canonical spelling the service resolved to, reusable verbatim in a follow-up call
...
- `dataset?: string` — present when a `stage: true` call registered a table. The `df_<id>` handle — pass directly to `eia_dataframe_query` SQL (`SELECT ... FROM df_<id>`). Every dataset a tenant stages lives in one shared canvas, so handles from different routes join by name.
- `canvas_preview_note?: string` — present when `total` exceeds the inline preview. On a `stage: true` call it names how many rows actually reached the canvas table, and where in the matching set they sit whenever the stage starts past row 1 — a staged count alone does not separate a tail from a prefix of the same query; when staging also stopped short of `total` — the `EIA_CANVAS_MAX_ROWS` cap, or an upstream page that didn't return — it says so and gives the offset to resume from. Where staging was not requested it places the inline page against `total` and names `stage: true` as the way to get SQL access to the rest, which is the only signal the caller has that the capability exists. Where no canvas is configured at all the lever does not exist, so it names `offset` paging and `CANVAS_PROVIDER_TYPE=duckdb` instead.
...
- `invalid_sort` (`ValidationError`) — a `sort` entry named a column the route does not sort by; recovery points at `data_columns[].id` and `facets[].id`
- `invalid_period` (`ValidationError`) — `start` or `end` was not in a period format the route accepts; recovery points at `frequencies[].format`
...
EIA rejects every bad parameter with the same 400, so the reason is read off the upstream message, which names the dimension verbatim — `Invalid data '…'`, `Invalid frequency '…'`, `Invalid sort '…'`, and `Invalid date format '…'` each match a pattern. `invalid_facet` is the unmatched fallback rather than a fifth pattern: EIA's `Invalid facet '…'` lands there, and so does any 400 shape not yet enumerated. A zero-row result is a success with a `notice`, not an error. A `length` above 5,000 is rejected by the input schema as `-32602 InvalidParams` before the handler runs, so it carries no `data.reason`. A 200 response carrying no `response` envelope is an upstream shape failure rather than a caller error, so it bubbles as a bare `ServiceUnavailable` with no `data.reason` instead of borrowing a declared reason whose code and `when` don't fit it.
...
| 5 | Analyze the staged set | SQL on `dataset`, when step 4 ran with `stage: true` |
...
| # | Action | Tool | Notes |
|:--|:-------|:-----|:------|
| 1 | Pull electricity retail sales | `eia_query_route` | `stage: true` — returns `dataset` (`df_abc`) |
| 2 | Pull natural gas spot prices | `eia_query_route` | `stage: true` — returns a second `dataset` (`df_xyz`) in the same canvas |
| 3 | Pull petroleum consumption by sector | `eia_query_route` | `stage: true` — returns a third `dataset` (`df_def`) |
...
- **DataCanvas spillover: opt-in or always-on?** → Opt-in via `CANVAS_PROVIDER_TYPE=duckdb`. DuckDB has no V8-isolate build, so Workers deployments would break if it were always attempted. `eia_query_route` checks canvas presence via `ctx.core.canvas?` at runtime and degrades to preview-only when absent; the canvas-only dataframe tools have nothing to degrade to and are gated at registration instead — see the registration-gate entry below.
...
- **Should canvas staging be automatic whenever a canvas exists?** → No — it is a per-call `stage: boolean` input, default `false`. Keying accumulation on bridge presence made every call on a large route pay for it: measured against the live API at `length: 3`, `seds` went 1.48 s → 9.42 s, `total-energy` 7.84 s → 42.17 s, and `electricity/facility-fuel` 2.25 s → 32.42 s, entirely to stage a prefix the response then tells the caller not to rely on. That cost lands hardest on the browse → describe → query discovery loop the tool descriptions prescribe. Two alternatives were declined: skipping accumulation when `total` far exceeds the cap introduces an untunable magic threshold and still charges a route whose `total` sits just over it, and staging only when `total <= EIA_CANVAS_MAX_ROWS` removes the ability to stage a bounded slice of a multi-million-row route — the capability #26 added. Under an explicit input the accumulation path runs unchanged when it runs, so #26's invariant holds: a staging call stages the whole match up to the cap, never the previewed page alone. The cost of an opt-in is discoverability, which is why the note on an unstaged call names `stage: true` rather than reusing the canvas-off wording.
- **Why gate the dataframe tools at registration rather than letting them fail at call time?** → On the documented default (`CANVAS_PROVIDER_TYPE` commented out) a client saw six tools, two of which failed every call — and a deployment that had turned `EIA_DATAFRAME_DROP_ENABLED` on saw seven, three of them dead — and the recovery hint — set a server environment variable — is an operator action the calling agent cannot take, so it read as a dead end rather than a next step. `eia_query_route` has something to degrade to; the canvas-only tools do not. `disabledTool()` keeps them on the landing page with the gate named while skipping registration, so the surface is four working tools. The gate reads `process.env.CANVAS_PROVIDER_TYPE` directly: `createApp()` composes `tools[]` before `setup(core)` runs, so `core.canvas` is not in hand, and `CANVAS_PROVIDER_TYPE` is core config, which never merges into `ServerConfigSchema`. `eia_dataframe_drop` carries the same gate beneath its own flag — a drop tool that cannot reach a canvas is the same dead advertisement — and the disabled card names whichever gate is actually unmet. Turning the canvas on by default was considered and declined: it would flip every local, `bunx`, and `.mcpb` install into a DuckDB path with a 1 GB default memory limit it did not ask for, and the `.mcpb` bundle deliberately ships without the platform-specific DuckDB binding.
- **Does dataframe provenance record the route the caller typed or the one it resolved to?** → The one it resolved to. `queryParams.route` is what `eia_dataframe_describe` reports as a staged table's source, and two spellings of one route recorded verbatim read as two different sources.
...
- **A facet value EIA sends without a name: drop it or label it?** → Label it, from the `alias` and then the `id`. Dropping on a null `id` *or* a null `name` was the original fix for a ZodError on `international`, where one `dataFlagId` value carries null in both fields — but the two are not the same failure. Without an `id` there is nothing to filter with, so the value is genuinely unusable; without a `name` only the label is missing, and EIA carries it in `alias` on exactly those values. The cost was invisible until a facet came along where every value has the shape: `electricity/state-electricity-profiles/meters`'s `technology` reported `(0 values)` while `facets[technology][]=Advanced Meter Infrastructure` returned 1,820 rows live, so the dimension read as unfilterable. Widening the output schema to accept null was rejected as the alternative: a null `name` in `structuredContent` is a second thing for every caller to handle, where the fallback resolves it once at the boundary that knows what EIA meant. Alias order beats id order because the alias is the label a reader wants (`Advanced Metering Infrastructure (AMI)` over the id it repeats), and the pair still renders once — the alias-suppression rule recognizes an alias that now equals the name.
...
- **Facet value fields are not all populated:** `id` arrives null on some values (`international`'s `dataFlagId`; 3 of 107 on `electricity/operating-generator-capacity`'s `balancing_authority_code`), and `name` is null or absent on others — every value of `electricity/state-electricity-profiles/meters`'s `technology` omits it and carries the label in `alias` instead. `alias` itself is optional and absent from whole routes, `international` among them.
